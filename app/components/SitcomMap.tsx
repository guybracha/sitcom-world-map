...
export default function SitcomMap() {
  ...
  const visibleShows = useMemo(() => {
    if (activeTag) return shows.filter((s) => s.tags.includes(activeTag));
    return shows;
  }, [activeTag]);
  ...
  useEffect(() => {
    ...
    import("leaflet").then((L) => {
      ...
      const map = L.map(container, { zoomControl: false }).setView([30, 10], 2);
  ...
  return (
    <div className="flex flex-col h-dvh bg-gray-50" dir="rtl" style={{ fontFamily: "system-ui, sans-serif" }}>
      ...
      <header className="bg-white border-b border-gray-200 px-3 py-2 flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-base font-medium text-gray-900 leading-tight truncate">🎬 מפת סיטקומים עולמית</h1>
            <p className="text-xs text-gray-400">{shows.length} סיטקומים · {ALL_TAGS.length} מדינות</p>
          </div>
          ...
          <div className="relative" ref={filterRef}>
            ...
            >
              ...
              <span>{activeTag ?? "סנן לפי מדינה"}</span>
              {activeTag && (
                <span
                  onClick={(e) => { e.stopPropagation(); selectTag(null); }}
                  className="mr-0.5 opacity-70 hover:opacity-100 font-bold"
                >×</span>
              )}
            </button>
            ...
            {filterOpen && (
              <div
                className="absolute left-0 top-full mt-1 bg-white border border-gray-200 rounded-xl shadow-xl z-50 p-3"
                style={{ width: "min(420px, 92vw)", direction: "rtl" }}
              >
                {/* All button */}
                <button
                  onClick={() => selectTag(null)}
                  className="w-full mb-2 px-3 py-1.5 rounded-lg text-sm border transition-colors text-right"
                  style={activeTag === null
                    ? { background: "#1f2937", color: "#fff", borderColor: "#1f2937" }
                    : { background: "#f9fafb", color: "#374151", borderColor: "#e5e7eb" }}
                >
                  הכל — {shows.length} סיטקומים
                </button>
                ...
                <div className="grid gap-1.5" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(110px, 1fr))" }}>
                  {ALL_TAGS.map((tag) => (
                    <button
                      ...
                      className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs border transition-colors text-right"
                      style={activeTag === tag
                    ...
                    >
                      ...
                      <span className="mr-auto opacity-60 flex-shrink-0">({tagCount(tag)})</span>
                    </button>
                  ))}
                </div>
              </div>
            ...
            {searchOpen ? (
              <div className="relative flex items-center">
                <input
                  ...
                  placeholder="חפש סיטקום, עיר, ערוץ..."
                  className="w-44 sm:w-60 border border-gray-300 rounded-lg pr-3 pl-8 py-1.5 text-sm text-gray-800 outline-none focus:border-blue-400"
                  style={{ direction: "rtl" }}
                />
                <button
                  onClick={() => { setSearchOpen(false); setQuery(""); }}
                  className="absolute left-2 text-gray-400 hover:text-gray-600 text-base leading-none"
                >×</button>
              </div>
            ) : (
              <button
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        {query.trim() && (
          ...
          >
            {filtered.length === 0 ? (
              <p className="text-sm text-gray-400 text-center py-4">לא נמצאו תוצאות</p>
            ) : (
              filtered.map((show) => (
                <button
                  key={show.name}
                  onClick={() => flyToShow(show)}
                  className="w-full flex items-start gap-3 px-4 py-3 hover:bg-gray-50 transition-colors border-b border-gray-100 last:border-0 text-right"
                >
                  <div
                  ...
                  />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-gray-900 truncate">{show.name}</p>
                    <p className="text-xs text-gray-400 truncate">{show.city} · {show.years}</p>
                  </div>
                  <span
        ...
        <div
          ...
          style={{ direction: "rtl" }}
        >
          {selected && <PanelContent show={selected} onClose={() => setSelected(null)} />}
        </div>
        ...
        {selected && (
          ...
          >
            ...
            <PanelContent show={selected} onClose={() => setSelected(null)} />
          </div>
        )}
...
function PanelContent({ show, onClose }: { show: Show; onClose: () => void }) {
  return (
    <>
      <div className="h-1.5 w-full flex-shrink-0 rounded-t-sm" style={{ background: TAG_COLORS[show.tags[0]] }} />
      <div className="p-5">
        <div className="flex items-start justify-between mb-2">
          <h2 className="text-base font-medium text-gray-900 leading-snug ml-2">{show.name}</h2>
          <button
            onClick={onClose}
        ...
        <p className="text-xs text-gray-400 mb-3 flex items-center gap-1">
          <span>📍</span><span>{show.city}</span>
        </p>

        <p className="text-sm text-gray-700 leading-relaxed mb-5">{show.desc}</p>

        <div className="space-y-2 border-t border-gray-100 pt-4">
          {([ ["שנים", show.years], ["ערוץ / רשת", show.network] ] as [string, string][]).map(([label, value]) => (
            <div key={label} className="flex justify-between text-sm">
              <span className="text-gray-400">{label}</span>
              <span className="text-gray-800 font-medium text-left">{value}</span>
            </div>
          ))}
          <div className="flex justify-between text-sm">
            <span className="text-gray-400">קטגוריה</span>
            <span
